...
> 最新版本：**v0.4.0**（2026-07-31） · [查看更新日志](CHANGELOG.md) ·
> [查看 GitHub Release](https://github.com/fyfsxkh/TokBrain/releases/tag/v0.4.0)
...
flowchart LR
    ...
    B --> C["逐条选择收藏夹<br/>默认：手动导入"]
    C --> D["确认进入待入库"]
    D --> E["用户主动开始入库"]
    E --> F["重新解析并刷新权限"]
    F -->|明确允许下载| G["完整媒体处理<br/>关键帧 / OCR / ASR"]
    F -->|拒绝或权限不明确| H["不下载完整视频<br/>字幕 / 独立音频 / 元数据"]
    G --> I["总结、分块与向量化"]
    H --> I
    I --> J["本地知识库"]
    J --> K["带来源的 RAG 问答"]
...
知识库。用户先为每条作品选择收藏夹（默认“手动导入”），确认后作品只进入“待入库”；
随后在导入页点击“入库（数量）”，或在知识库手动点击“开始入库”，才会刷新作品信息
并开始处理。如果权限发生变化，处理范围会随之收窄。
...
4. 为每条结果选择收藏夹（默认“手动导入”），勾选后确认加入“待入库”；预检与确认
   本身不会产生 AI 用量。
5. 确认后导入页按钮会切换为“入库（数量）”，可直接处理刚才确认的作品；也可前往
   “知识库 → 待入库”单条或批量开始入库。
6. 在“知识库”查看总结，或在“对话”中基于已入库内容提问。选择某个收藏夹后可为其
   设置专属总结提示词；未设置时使用设置页的全局提示词。

页面切换不会清空已经完成的预检结果；预检条目可单独删除。知识库中的待入库、已入库、
异常和归档作品都可以永久删除，已经生成的总结、索引及本地资产会同步清理。
...
首次启动数据库结构 **schema v5** 时，程序会先创建带时间戳的 SQLite 备份，再迁移
历史作品、总结、知识块、关键帧、用量和本地收藏夹；从 schema v4 升级时会保留已有
预检与入库队列。应用重启不会自动恢复遗留的平台访问或处理任务。
...
- [最新 GitHub Release](https://github.com/fyfsxkh/TokBrain/releases/tag/v0.4.0)
